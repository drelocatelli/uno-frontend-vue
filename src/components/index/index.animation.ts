...
class IndexFx {
    static loading() {
        const start = () => {
            ...
            timeline.to('.initialLogo', { duration: 0, position: 'absolute', bottom: '30%' });
            timeline.to('.logo', {
                duration: 2,
                y: '50%',
                ease: 'back',
            ...
            timeline2.to('.initialLogo', { duration: .5, opacity: 1, ease: 'circ' });
            timeline2.to('.logo', { duration: 1, y: '0', ease: 'circ' });
        };

        const end = () => {
            const timeline = gsap.timeline();
            timeline.to('.initialLogo', {duration: .3, opacity: 0, display: 'none'})
